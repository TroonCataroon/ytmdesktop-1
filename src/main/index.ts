...
function getTrayIconPath() {
  ...
  // Use assetFolder which is properly defined at line 53 to ensure correct path resolution
  const iconsDir = process.env.NODE_ENV === "development" ? path.join(assetFolder, "icons") : assetFolder;
...
app.on("ready", async () => {
  ...
  ipcMain.on("app:restartApplicationForUpdate", event => {
    ...
    // Track update installation in Sentry if enabled
    if (SENTRY_CONFIG.updateMonitoring?.enabled && SENTRY_CONFIG.updateMonitoring.trackEvents.updateInstalled) {
      const updateInfo = memoryStore.get("updateInfo");
      sentryIntegration.captureMessage("Auto-update installing", "info", {
        currentVersion: app.getVersion(),
