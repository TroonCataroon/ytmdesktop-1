...
app.on("ready", async () => {
  ...
  ipcMain.handle("plugins:getAll", () => {
    return pluginManager.getAllPlugins().map(p => ({
      ...
      version: p.version,
      author: p.author,
      enabled: pluginManager.isPluginEnabled(p.id)
    }));
  });

  ipcMain.handle("plugins:getSettingsSchema", (event, pluginId) => {
    return pluginManager.getPluginSettingsSchema(pluginId);
  });

  ipcMain.handle("plugins:getSettings", (event, pluginId) => {
    const plugin = pluginManager.getPlugin(pluginId);
    return plugin ? plugin.settings : null;
  });

  ipcMain.handle("plugins:updateSettings", (event, pluginId, settings) => {
